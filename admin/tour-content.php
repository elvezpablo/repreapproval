...
{
	$update=mysql_query("UPDATE  take_tour SET status='0' WHERE id='".$_REQUEST['upd_id']."' ");
		if($update)
	{
		header("LOCATION:tour-content.php?msg=updated");
	}	
}
 
 if(isset($_REQUEST['action']) && $_REQUEST['action']=='update_red')
{
	$update=mysql_query("UPDATE  take_tour SET status='1' WHERE id='".$_REQUEST['update_id']."' ");
	if($update)
	{
		header("LOCATION:tour-content.php?msg=updated");
	}	
...
{
	$delete=mysql_query("delete from  take_tour  where id='".$_REQUEST['dlt_id']."'");
	if($delete)
	{
		header("LOCATION:tour-content.php?msg=deleted");
	}	
}
...
{
	$insert_pages=mysql_query("INSERT INTO  take_tour set title='".addslashes($_REQUEST['title'])."',content='".addslashes($_REQUEST['content'])."'");
		if($insert_pages)
		{
			header("LOCATION:tour-content.php?msg=Added");
		}
		else
		{
			echo "ERROR";
...
{
$insert_pages=mysql_query("update  take_tour set title='".addslashes($_REQUEST['edittitle'])."',content='".addslashes($_REQUEST['editcontent'])."' where id='".$_REQUEST['edit_id']."'");
		if($insert_pages)
		{
			header("LOCATION:tour-content.php?msg=updated");
		}
               ...
               <div class="span12">
                  ...
                  <!-- BEGIN PAGE TITLE & BREADCRUMB-->
                   ...
                   <ul class="breadcrumb">
                       ...
                       <li class="active">
                        <a href="javascript:;"> Tour Content</a>
                ...
                <div class="span12">
                    <!-- BEGIN SAMPLE FORMPORTLET-->
                        ...
                        <div class="widget-body">
                            <form action="tour-content.php" class="form-horizontal" method="post" enctype="multipart/form-data">
...
$sBasePath = substr( $sBasePath, 0, strpos( $sBasePath, "tour-content.php"));
...
?>
                                ...
                                <div class="form-actions">
                                    ...
                                    <button type="button" class="btn" onClick="window.location='tour-content.php';"><i class=" icon-remove"></i>Cancel</button>
                                </div>
                            </form>
                            </div></div>
                            <?php } else if(isset($_REQUEST['action']) and $_REQUEST['action']=='edit'){
								 $select_edit=mysql_query("SELECT * FROM  take_tour WHERE id='".$_REQUEST['edit_id']."'");
...
?>
                            ...
                            <?php } else {
								...
								$list_pages=mysql_query("SELECT * FROM  take_tour");
                                 $count=mysql_num_rows($list_pages);
								?>
                            <!-- END FORM--><div class="btn-group">
                                         <button  onClick="window.location='tour-content.php?action=add';" class="btn green" id="editable-sample_new">
...
while($data=mysql_fetch_object($list_pages))
{
 ?>
                                ...
                                <?php if($data->status==1)
		 {
			...
			<a class="btn btn-small btn-success" href="tour-content.php?action=update_green&upd_id=<?php echo $data->id?>">Active</a></td><?php
		 }
		else
		 {
			?><td class="hidden-phone" >
			<a class="btn btn-small btn-danger" href="tour-content.php?action=update_red&update_id=<?php echo $data->id?>">Deactive</a></td><?php
		 }?>
    
                                <td class="hidden-phone"><a class="btn btn-small btn-primary" href="tour-content.php?action=edit&edit_id=<?php echo $data->id;?>"><i class="icon-pencil icon-white"></i> Edit</a></td>
...
$('.dell').live('click',function(){
  ...
  apprise("Are you sure you want to delete?", {'verify': true, 'animate': true, 'textYes': 'Yes', 'textNo': 'No' },
    function (r) {
  if (r) {
	  window.location.href='tour-content.php?action=delete&dlt_id='+DelId;
	  }
   else
   {
    return false;
