...
{
	$insert_pages=mysql_query("INSERT INTO  testimonial set name='".addslashes($_REQUEST['title'])."',content='".addslashes($_REQUEST['content'])."'");
		if($insert_pages)
		{
			header("LOCATION:contact.php?msg=Added");
		}
		else
		{
			echo "ERROR";
...
{
$insert_pages=mysql_query("update  testimonial set name='".addslashes($_REQUEST['edittitle'])."',content='".addslashes($_REQUEST['editcontent'])."' where id='".$_REQUEST['edit_id']."'");
		if($insert_pages)
		{
			header("LOCATION:contact.php?msg=updated");
		}
...
$sBasePath = substr( $sBasePath, 0, strpos( $sBasePath, "contact.php"));
...
?>
                                ...
                                <div class="form-actions">
                                    ...
                                    <button type="button" class="btn" onClick="window.location='contact.php';"><i class=" icon-remove"></i>Cancel</button>
                                </div>
                            </form>
                            </div></div>
                            ...
                            <form action="#" class="form-horizontal" method="post" enctype="multipart/form-data">
                             <input type="hidden" value="<?php echo $_REQUEST['edit_id']?>" name="edit_id"  class="span12"  />
                                ...
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                        <input type="text" placeholder="Text" required name="edittitle" id="edittitle" class="input-xxlarge" value="<?php echo stripslashes($data_edit['name']); ?>" />
                                    </div>
                                </div>
...
?>
                            ...
                            <!-- END FORM--><div class="btn-group">
                                         <button  onClick="window.location='contact.php?action=add';" class="btn green" id="editable-sample_new">
...
while($data=mysql_fetch_object($list_pages))
{
 ?>
                                ...
                                <?php if($data->status==1)
		 {
			...
			<a class="btn btn-small btn-success" href="contact.php?action=update_green&upd_id=<?php echo $data->id?>">Active</a></td><?php
		 }
		else
		 {
			?><td class="hidden-phone" >
			<a class="btn btn-small btn-danger" href="contact.php?action=update_red&update_id=<?php echo $data->id?>">Deactive</a></td><?php
		 }?>
    
                                <td class="hidden-phone"><a class="btn btn-small btn-primary" href="contact.php?action=edit&edit_id=<?php echo $data->id;?>"><i class="icon-pencil icon-white"></i> Edit</a></td>
...
$('.dell').live('click',function(){
  ...
  apprise("Are you sure you want to delete?", {'verify': true, 'animate': true, 'textYes': 'Yes', 'textNo': 'No' },
    function (r) {
  if (r) {
	  window.location.href='contact.php?action=delete&dlt_id='+DelId;
	  }
   else
   {
    return false;
