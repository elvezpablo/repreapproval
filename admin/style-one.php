...
{
	$update=mysql_query("UPDATE  new_page1 SET status='0' WHERE id='".$_REQUEST['upd_id']."' ");
		if($update)
	{
		header("LOCATION:style-one.php?msg=updated");
	}	
}
 
 if(isset($_REQUEST['action']) && $_REQUEST['action']=='update_red')
{
	$update=mysql_query("UPDATE  new_page1 SET status='1' WHERE id='".$_REQUEST['update_id']."' ");
	if($update)
	{
		header("LOCATION:style-one.php?msg=updated");
	}	
...
{
	//echo "select pagename from new_page1 where id='".$_REQUEST['dlt_id']."'";die;
	$pageNameDel=mysql_fetch_assoc(mysql_query("select pagename from new_page1 where id='".$_REQUEST['dlt_id']."'"));
	...
	$delete=mysql_query("delete from  new_page1  where id='".$_REQUEST['dlt_id']."'");
	if($delete)
	{
		header("LOCATION:style-one.php?msg=deleted");
	}	
}
...
if(isset($_REQUEST['submit']) && $_REQUEST['submit']=='Add')
{
	...
	function clean_url($text)
	{
	 ...
	 $page= str_replace(" ", "_",$_REQUEST['pagename']);
	  ...
	  $pgname=$pagename.".php";
      $sel1 = mysql_query("select * from new_page1 where pagename='".$pgname."'"); 
	 ...
	 if($numrow1==0 and $exists=='12345')
	{
		  	 ...
		  	 $insert_sql=mysql_query("insert into new_page1 set pagename='".$pgname."',metatitle ='".addslashes($_REQUEST['metatitle'])."',metakeyword='".addslashes($_REQUEST['metakeyword'])."',metadescription ='".addslashes($_REQUEST['metadescription'])."',pagetitle='".addslashes($_REQUEST['pagetitle'])."',pagesubtitle='".$_REQUEST['pagesubtitle']."',  content='".addslashes($_REQUEST['content'])."',url='".addslashes($_REQUEST['url'])."',smallcontent='".$_REQUEST['urltext']."'"); 
			 copy("../create_pages1.php","../$pgname");
       if($insert_sql)
		   {
			   header("location:style-one.php?msg=Added");
		   
}
	 }
	  else {
				 header("location:style-one.php?msg=Already");
				}
...
$insert_pages=mysql_query("update  new_page1 set metatitle='".addslashes($_REQUEST['metatitle'])."',metakeyword='".addslashes($_REQUEST['metakeyword'])."',metadescription='".addslashes($_REQUEST['metadescription'])."',pagetitle='".addslashes($_REQUEST['pagetitle'])."',content='".addslashes($_REQUEST['content'])."',pagesubtitle='".$_REQUEST['pagesubtitle']."',url='".addslashes($_REQUEST['url'])."',smallcontent='".$_REQUEST['urltext']."' where id='".$_REQUEST['edit_id']."'");
		if($insert_pages)
		{
			header("LOCATION:style-one.php?msg=updated");
		}
            ...
            <div class="row-fluid">
               <div class="span12">
                  ...
                  <!-- BEGIN PAGE TITLE & BREADCRUMB-->
                   <h3 class="page-title">
                     Manage Style One Pages <i>(using a full width column layout)</i>
                   </h3>
                   <ul class="breadcrumb">
                       <li>
                        ...
                        <div class="widget-body">
                            ...
                            <div class="control-group">
                                ...
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                 <input type="text" placeholder="URL Link for Call-to-action" id="url"   name="url" class="input-xxlarge" value="" />
                                    </div>
                                </div>
                                ...
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                 <input type="text" placeholder="Text for Call-to-action" id="urltext"   name="urltext" class="input-xxlarge" value="" />
                                    </div>
                                </div>
...
$sBasePath = substr( $sBasePath, 0, strpos( $sBasePath, "style-one.php"));
...
?>
                                ...
                                <div class="form-actions">
                                    ...
                                    <button type="button" class="btn" onClick="window.location='style-one.php';"><i class=" icon-remove"></i>Cancel</button>
                                </div>
                            </form>
                            </div></div>
                            <?php } else if(isset($_REQUEST['action']) and $_REQUEST['action']=='edit'){
								 $select_edit=mysql_query("SELECT * FROM  new_page1 WHERE id='".$_REQUEST['edit_id']."'");
                            ...
                            <form action="" class="form-horizontal" method="post" enctype="multipart/form-data" id="myform123">
                             <input type="hidden" value="<?php echo $_REQUEST['edit_id']?>" name="edit_id"  class="span12"  />
                                ...
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                 <input type="text" placeholder="URL Link for Call-to-action" id="url"   name="url" class="input-xxlarge" value="<?php echo stripslashes($data_edit['url']);?>" />
                                    </div>
                                </div>
                                ...
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                 <input type="text" placeholder="Text for Call-to-action" id="urltext"   name="urltext" class="input-xxlarge" value="<?php echo stripslashes($data_edit['smallcontent']);?>" />
                                    </div>
                                </div>
                                <div class="control-group">
                                    <label class="control-label"> Content</label>
          ...
          <button type="submit" class="btn btn-success" id="submit" name="submit" value="Update"><i class="icon-ok"></i> Save</button>
           ...
           <button type="button" class="btn preview_add" >Preview</button>
                            ...
                            <?php } else {
								...
								$list_pages=mysql_query("SELECT * FROM  new_page1 order by pagename");
                                 $count=mysql_num_rows($list_pages);
								?>
                            <!-- END FORM--><div class="btn-group">
                                         <button  onClick="window.location='style-one.php?action=add';" class="btn green" id="editable-sample_new">
                                             Add New <i class="icon-plus"></i>
                                         </button>
                                     </div>
                                     <p></p>
                                     <div class="widget red">
                                      <input type="hidden" name="tablename" id="tablename" value="new_page1"/>
                            <div class="widget-title">
...
while($data=mysql_fetch_object($list_pages))
{
 ?>
                            <tr class="odd gradeX widget" id="arrayorder_<?php echo $data->id; ?>">
                                ...
                                <td class="hidden-phone"><a class="btn btn-small btn-primary" href="style-one.php?action=edit&edit_id=<?php echo $data->id;?>"><i class="icon-pencil icon-white"></i> Edit</a></td>
...
$('.dell').live('click',function(){
  ...
  apprise("Are you sure you want to delete?", {'verify': true, 'animate': true, 'textYes': 'Yes', 'textNo': 'No' },
    function (r) {
  if (r) {
	  window.location.href='style-one.php?action=delete&dlt_id='+DelId;
	  }
   else
   {
    return false;
...
var urltext=$('#urltext').val();
		 $.ajax({
			 type:"POST",
			 url:'ajax_previewadd1.php?metatitle='+metatitle+'&metakeyword='+metakeyword+'&metadescription='+metadescription+'&pagetitle='+pagetitle+'&pagesubtitle='+pagesubtitle+'&tester1='+sValue+'&urltext='+urltext+'&url='+url,
			  data:fd,
			  processData: false,
              contentType: false,
			 success:function(html){
				 if(html==50)
					 {
						window.open('../previewlink1.php', '', 'height=800,width=800,scrollbars=yes');
					 }
					 else
					 {
					 }
