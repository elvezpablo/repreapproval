...
{
	$update=mysql_query("UPDATE home_scrrenshot SET status='0' WHERE id='".$_REQUEST['upd_id']."' ");
		if($update)
	{
		header("LOCATION:home-screenshot.php?msg=updated");
	}	
}
 
 if(isset($_REQUEST['action']) && $_REQUEST['action']=='update_red')
{
	$update=mysql_query("UPDATE home_scrrenshot SET status='1' WHERE id='".$_REQUEST['update_id']."' ");
	if($update)
	{
		header("LOCATION:home-screenshot.php?msg=updated");
	}	
...
if(isset($_REQUEST['action']) && $_REQUEST['action']=='delete')
{
	...
	$delete=mysql_query("delete from home_scrrenshot  where id='".$_REQUEST['dlt_id']."'");
	if($delete)
	{
		header("LOCATION:home-screenshot.php?msg=deleted");
	}	
}
...
if(isset($_REQUEST['submit']) && $_REQUEST['submit']=='Add')
{
	...
	$insert_pages=mysql_query("INSERT INTO home_scrrenshot set image='".$img."', img_alt='".addslashes($_REQUEST['imgalt'])."',img_title='".addslashes($_REQUEST['imgtitle'])."', 	title='".addslashes($_REQUEST['text1'])."',subtitle='".addslashes($_REQUEST['text2'])."',url='".addslashes($_REQUEST['text3'])."',urltext='".addslashes($_REQUEST['text4'])."'");
		if($insert_pages)
		{
			header("LOCATION:home-screenshot.php?msg=Added");
		}
...
if(isset($_REQUEST['submit']) && $_REQUEST['submit']=="Update")
{
	...
	$insert_pages=mysql_query("update home_scrrenshot set image='".$img."', img_alt='".addslashes($_REQUEST['imgalt'])."',img_title='".addslashes($_REQUEST['imgtitle'])."',	title='".addslashes($_REQUEST['text1'])."',subtitle='".addslashes($_REQUEST['text2'])."',url='".addslashes($_REQUEST['text3'])."',urltext='".addslashes($_REQUEST['text4'])."' where id='".$_REQUEST['edit_id']."'");
		if($insert_pages)
		{
			header("LOCATION:home-screenshot.php?msg=updated");
		}
 ...
 <!-- BEGIN CONTAINER -->
   <div id="container" class="row-fluid">
                ...
                <div class="span12">
                    <!-- BEGIN SAMPLE FORMPORTLET-->
                        ...
                        <div class="widget-body">
                            <form action="home-screenshot.php" class="form-horizontal" method="post" enctype="multipart/form-data">
                            ...
                            <div class="control-group">
                                ...
                                <div class="form-actions">
                                    ...
                                    <button type="button" class="btn" onClick="window.location='home-screenshot.php';"><i class=" icon-remove"></i>Cancel</button>
                                </div>
                            </form>
                            </div></div>
                            <?php } else if(isset($_REQUEST['action']) and $_REQUEST['action']=='edit'){
								 $select_edit=mysql_query("SELECT * FROM home_scrrenshot WHERE id='".$_REQUEST['edit_id']."'");
	  $data_edit=mysql_fetch_assoc($select_edit);
                            ...
                            <form action="home-screenshot.php" class="form-horizontal" method="post" enctype="multipart/form-data">
                             <input type="hidden" value="<?php echo $_REQUEST['edit_id']?>" name="edit_id"   />
                                ...
                                <div class="control-group">
                                 ...
                                 <div class="control-group">
                                    ...
                                    <div class="controls">
                                        ...
                                        <input type="text" placeholder="Text" class="input-xlarge" name="text1" id="text1" value="<?php echo $data_edit['title']  ?>" required />
                                ...
                                <div class="control-group">
                                    <label class="control-label">Description</label>
                                    <div class="controls">
                                        <input type="text" placeholder="Text" class="input-xlarge" name="text2" id="text2" value="<?php echo $data_edit['subtitle']  ?>" required />
                                    </div>
                                </div>
                                ...
                                <div class="control-group">
                                    <label class="control-label">URL</label>
                                    <div class="controls">
                                       <input type="text" placeholder="Text" class="input-xlarge" name="text3" id="text3" value="<?php echo $data_edit['url']  ?>" required />
                                    </div>
                                </div>
                                  ...
                                  <div class="control-group">
                                    <label class="control-label">URL Text</label>
                                    <div class="controls">
                                       <input type="text" placeholder="Text" class="input-xlarge" name="text4" id="text4" value="<?php echo $data_edit['urltext']  ?>" required />
                                    </div>
                                </div>
                            ...
                            <?php } else {
								$list_pages=mysql_query("SELECT * FROM home_scrrenshot order by order_id");
                                 $count=mysql_num_rows($list_pages);
								?>
                            <!-- END FORM--><div class="btn-group">
                                         <button  onClick="window.location='home-screenshot.php?action=add';" class="btn green" id="editable-sample_new">
                                             Add New <i class="icon-plus"></i>
                                         </button>
                                     </div>
                                     <p></p>
                                     <div class="widget red">
                                       <input type="hidden" name="tablename" id="tablename" value="home_scrrenshot"/>
                            <div class="widget-title">
...
while($data=mysql_fetch_object($list_pages))
{
 ?>
                                ...
                                <?php if($data->status==1)
		 {
			...
			<a class="btn btn-small btn-success" href="home-screenshot.php?action=update_green&upd_id=<?php echo $data->id?>">Active</a></td><?php
		 }
		else
		 {
			?><td class="hidden-phone" >
			<a class="btn btn-small btn-danger" href="home-screenshot.php?action=update_red&update_id=<?php echo $data->id?>">Deactive</a></td><?php
		 }?>
    
                                <td class="hidden-phone"><a class="btn btn-small btn-primary" href="home-screenshot.php?action=edit&edit_id=<?php echo $data->id;?>"><i class="icon-pencil icon-white"></i> Edit</a></td>
...
$('.dell').live('click',function(){
  ...
  apprise("Are you sure you want to delete?", {'verify': true, 'animate': true, 'textYes': 'Yes', 'textNo': 'No' },
    function (r) {
  if (r) {
	  window.location.href='home-screenshot.php?action=delete&dlt_id='+DelId;
	  }
   else
   {
    return false;
