...
{
	$update=mysql_query("UPDATE take_tour_img SET status='0' WHERE id='".$_REQUEST['upd_id']."' ");
		if($update)
	{
		header("LOCATION:tour-image.php?msg=updated");
	}	
}
 
 if(isset($_REQUEST['action']) && $_REQUEST['action']=='update_red')
{
	$update=mysql_query("UPDATE take_tour_img SET status='1' WHERE id='".$_REQUEST['update_id']."' ");
	if($update)
	{
		header("LOCATION:tour-image.php?msg=updated");
	}	
...
{
	$delete=mysql_query("delete from take_tour_img  where id='".$_REQUEST['dlt_id']."'");
	if($delete)
	{
		header("LOCATION:tour-image.php?msg=deleted");
	}	
}
...
if(!empty($_FILES['img']['name']))
	...
	$insert_pages=mysql_query("INSERT INTO take_tour_img set img_title='".addslashes($_REQUEST['title'])."',img_alt='".addslashes($_REQUEST['alt'])."',image='".$backimg."'");
		if($insert_pages)
		{
			header("LOCATION:tour-image.php?msg=Added");
		}
		else
		{
			echo "ERROR";
...
$insert_pages=mysql_query("update take_tour_img set img_title='".addslashes($_REQUEST['edittitle'])."',img_alt='".addslashes($_REQUEST['alt'])."',image='".$backimg."' where id='".$_REQUEST['edit_id']."'");
		if($insert_pages)
		{
			header("LOCATION:tour-image.php?msg=updated");
		}
 ...
 <!-- BEGIN CONTAINER -->
   <div id="container" class="row-fluid">
            ...
            <div class="row-fluid">
               <div class="span12">
                  ...
                  <!-- BEGIN PAGE TITLE & BREADCRUMB-->
                   <h3 class="page-title">
                     Manage Take A Tour Page <i>(tour.php)</i>
                   </h3>
                   <ul class="breadcrumb">
                       <li>
                ...
                <div class="span12">
                    <!-- BEGIN SAMPLE FORMPORTLET-->
                        ...
                        <div class="widget-body">
                            <form action="tour-image.php" class="form-horizontal" method="post" enctype="multipart/form-data">
                            ...
                            <div class="control-group">
                                ...
                                <div class="form-actions">
                                    ...
                                    <button type="button" class="btn" onClick="window.location='tour-image.php';"><i class=" icon-remove"></i>Cancel</button>
                                </div>
                            </form>
                            </div></div>
                            <?php } else if(isset($_REQUEST['action']) and $_REQUEST['action']=='edit'){
								 $select_edit=mysql_query("SELECT * FROM take_tour_img WHERE id='".$_REQUEST['edit_id']."'");
	  $data_edit=mysql_fetch_assoc($select_edit);
                            ...
                            <form action="tour-image.php" class="form-horizontal" method="post" enctype="multipart/form-data">
                             ...
                             <div class="control-group">
                                ...
                                <div class="control-group">
                                    <label class="control-label">Title</label>
                                    <div class="controls">
                                        <input type="text" placeholder="Text" required name="edittitle" id="edittitle" class="input-xxlarge" value="<?php echo stripslashes($data_edit['img_title']); ?>" />
                                    </div>
                                </div>
                                
                                <div class="control-group">
                                    <label class="control-label">Alt</label>
                                    <div class="controls"> <input type="text" placeholder="alt " required name="alt" id="alt" class="input-xxlarge" value="<?php echo stripslashes($data_edit['img_alt']); ?>" />
                                    </div>
                                </div>
                            ...
                            <?php }  else {
								//echo "SELECT * FROM  take_tour_img order by id ASC";die;
								$list_pages=mysql_query("SELECT * FROM  take_tour_img order by order_id");
                                 $count=mysql_num_rows($list_pages);
								?>
                            <!-- END FORM--><div class="btn-group">
                                         <button  onClick="window.location='tour-image.php?action=add';" class="btn green" id="editable-sample_new">
                                             Add New <i class="icon-plus"></i>
                                         </button>
                                     </div>
                                     <p></p>
                                     <div class="widget red">
                                      <input type="hidden" name="tablename" id="tablename" value="take_tour_img"/>
                            <div class="widget-title">
 ...
 ?>
                           <tr class="odd gradeX widget" id="arrayorder_<?php echo $data->id; ?>" >
                                ...
                                <td><?php echo stripslashes($data->img_title);?></td>
                                 ...
                                 <td><img src="../userfiles/<?php echo stripslashes($data->image);?>" width="100" height="100"/></td>
                                <?php if($data->status==1)
		 {
			?><td class="hidden-phone">
			<a class="btn btn-small btn-success" href="tour-image.php?action=update_green&upd_id=<?php echo $data->id?>">Active</a></td><?php
		 }
		else
		 {
			?><td class="hidden-phone" >
			<a class="btn btn-small btn-danger" href="tour-image.php?action=update_red&update_id=<?php echo $data->id?>">Deactive</a></td><?php
		 }?>
    
                                <td class="hidden-phone"><a class="btn btn-small btn-primary" href="tour-image.php?action=edit&edit_id=<?php echo $data->id;?>"><i class="icon-pencil icon-white"></i> Edit</a></td>
...
$('.dell').live('click',function(){
  ...
  apprise("Are you sure you want to delete?", {'verify': true, 'animate': true, 'textYes': 'Yes', 'textNo': 'No' },
    function (r) {
  if (r) {
	  window.location.href='tour-image.php?action=delete&dlt_id='+DelId;
	  }
   else
   {
    return false;
