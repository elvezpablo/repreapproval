...
{
	$update=mysql_query("UPDATE power_right_content SET status='0' WHERE id='".$_REQUEST['upd_id']."' ");
		if($update)
	{
		header("LOCATION:powerright-content.php?msg=updated");
	}	
}
 
 if(isset($_REQUEST['action']) && $_REQUEST['action']=='update_red')
{
	$update=mysql_query("UPDATE power_right_content SET status='1' WHERE id='".$_REQUEST['update_id']."' ");
	if($update)
	{
		header("LOCATION:powerright-content.php?msg=updated");
	}	
...
if(isset($_REQUEST['action']) && $_REQUEST['action']=='delete')
{
	...
	$delete=mysql_query("delete from power_right_content  where id='".$_REQUEST['dlt_id']."'");
	if($delete)
	{
		header("LOCATION:powerright-content.php?msg=deleted");
	}	
}
...
if(isset($_REQUEST['submit']) && $_REQUEST['submit']=='Add')
{
	...
	$insert_pages=mysql_query("INSERT INTO power_right_content set image='".$img."', img_alt='".addslashes($_REQUEST['imgalt'])."',img_title='".addslashes($_REQUEST['imgtitle'])."',content='".addslashes($_REQUEST['content'])."'");
		if($insert_pages)
		{
			header("LOCATION:powerright-content.php?msg=Added");
		}
...
if(isset($_REQUEST['submit']) && $_REQUEST['submit']=="Update")
{
	...
	$insert_pages=mysql_query("update power_right_content set image='".$img."', img_alt='".addslashes($_REQUEST['imgalt'])."',img_title='".addslashes($_REQUEST['imgtitle'])."',content='".addslashes($_REQUEST['content'])."' where id='".$_REQUEST['edit_id']."'");
		if($insert_pages)
		{
			header("LOCATION:powerright-content.php?msg=updated");
		}
            ...
            <div class="row-fluid">
               <div class="span12">
                  ...
                  <!-- BEGIN PAGE TITLE & BREADCRUMB-->
                   <h3 class="page-title">
                     Manage Right Column on Powerful Letter Page <i>(powerfulpreapprovalletters.php)</i>
                   </h3>
                   <ul class="breadcrumb">
                       <li>
                ...
                <div class="span12">
                    <!-- BEGIN SAMPLE FORMPORTLET-->
                        ...
                        <div class="widget-body">
                            <form action="powerright-content.php" class="form-horizontal" method="post" enctype="multipart/form-data">
...
$sBasePath = substr( $sBasePath, 0, strpos( $sBasePath, "powerright-content.php"));
...
?>
                                ...
                                <div class="form-actions">
                                    ...
                                    <button type="button" class="btn" onClick="window.location='powerright-content.php';"><i class=" icon-remove"></i>Cancel</button>
                                </div>
                            </form>
                            </div></div>
                            <?php } else if(isset($_REQUEST['action']) and $_REQUEST['action']=='edit'){
								 $select_edit=mysql_query("SELECT * FROM power_right_content WHERE id='".$_REQUEST['edit_id']."'");
...
?>
                            ...
                            <?php } else {
								$list_pages=mysql_query("SELECT * FROM power_right_content");
                                 $count=mysql_num_rows($list_pages);
								?>
                            <!-- END FORM--><div class="btn-group">
                                         <button  onClick="window.location='powerright-content.php?action=add';" class="btn green" id="editable-sample_new">
                                             Add New <i class="icon-plus"></i>
                                         </button>
                                     </div>
                                     <p></p>
                                     <div class="widget red">
                                       <input type="hidden" name="tablename" id="tablename" value="power_right_content"/>
                            <div class="widget-title">
...
while($data=mysql_fetch_object($list_pages))
{
 ?>
                                ...
                                <?php if($data->status==1)
		 {
			...
			<a class="btn btn-small btn-success" href="powerright-content.php?action=update_green&upd_id=<?php echo $data->id?>">Active</a></td><?php
		 }
		else
		 {
			?><td class="hidden-phone" >
			<a class="btn btn-small btn-danger" href="powerright-content.php?action=update_red&update_id=<?php echo $data->id?>">Deactive</a></td><?php
		 }?>
    
                                <td class="hidden-phone"><a class="btn btn-small btn-primary" href="powerright-content.php?action=edit&edit_id=<?php echo $data->id;?>"><i class="icon-pencil icon-white"></i> Edit</a></td>
...
$('.dell').live('click',function(){
  ...
  apprise("Are you sure you want to delete?", {'verify': true, 'animate': true, 'textYes': 'Yes', 'textNo': 'No' },
    function (r) {
  if (r) {
	  window.location.href='powerright-content.php?action=delete&dlt_id='+DelId;
	  }
   else
   {
    return false;
