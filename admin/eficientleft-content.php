...
{
	$update=mysql_query("UPDATE efficient_letter_left SET status='0' WHERE id='".$_REQUEST['upd_id']."' ");
		if($update)
	{
		header("LOCATION:eficientleft-content.php?msg=updated");
	}	
}
 
 if(isset($_REQUEST['action']) && $_REQUEST['action']=='update_red')
{
	$update=mysql_query("UPDATE efficient_letter_left SET status='1' WHERE id='".$_REQUEST['update_id']."' ");
	if($update)
	{
		header("LOCATION:eficientleft-content.php?msg=updated");
	}	
...
if(isset($_REQUEST['action']) && $_REQUEST['action']=='delete')
{
	...
	$delete=mysql_query("delete from efficient_letter_left  where id='".$_REQUEST['dlt_id']."'");
	if($delete)
	{
		header("LOCATION:eficientleft-content.php?msg=deleted");
	}	
}
...
if(isset($_REQUEST['submit']) && $_REQUEST['submit']=='Add')
{
	...
	$insert_pages=mysql_query("INSERT INTO efficient_letter_left set image='".$img."', img_alt='".addslashes($_REQUEST['imgalt'])."',img_title='".addslashes($_REQUEST['imgtitle'])."',page_title='".addslashes($_REQUEST['text1'])."'");
		if($insert_pages)
		{
			header("LOCATION:eficientleft-content.php?msg=Added");
		}
...
if(isset($_REQUEST['submit']) && $_REQUEST['submit']=="Update")
{
	...
	$insert_pages=mysql_query("update efficient_letter_left set image='".$img."', img_alt='".addslashes($_REQUEST['imgalt'])."',img_title='".addslashes($_REQUEST['imgtitle'])."',page_title='".addslashes($_REQUEST['text1'])."' where id='".$_REQUEST['edit_id']."'");
		if($insert_pages)
		{
			header("LOCATION:eficientleft-content.php?msg=updated");
		}
 ...
 <!-- BEGIN CONTAINER -->
   <div id="container" class="row-fluid">
            ...
            <div class="row-fluid">
               <div class="span12">
                  ...
                  <!-- BEGIN PAGE TITLE & BREADCRUMB-->
                   <h3 class="page-title">
                     Manage Left Column on Efficient Letter <i>(efficientpreapprovalletters.php)</i>
                   </h3>
                   <ul class="breadcrumb">
                       <li>
                       ...
                       <li>
                           <a href="javascript:;">Efficient Letter Page</a>
                           <span class="divider">/</span>
                       </li>
                       <li class="active">
                         Left Content
                ...
                <div class="span12">
                    <!-- BEGIN SAMPLE FORMPORTLET-->
                        ...
                        <div class="widget-body">
                            <form action="eficientleft-content.php" class="form-horizontal" method="post" enctype="multipart/form-data">
                              <div class="control-group">
                                    <label class="control-label">Page Title</label>
                                    <div class="controls">
                                    
                                        <input type="text" placeholder="Title" class="input-xlarge" name="text1" id="text1" value="" required />
                            ...
                            <div class="control-group">
                                ...
                                <div class="form-actions">
                                    ...
                                    <button type="button" class="btn" onClick="window.location='eficientleft-content.php';"><i class=" icon-remove"></i>Cancel</button>
                                </div>
                            </form>
                            </div></div>
                            <?php } else if(isset($_REQUEST['action']) and $_REQUEST['action']=='edit'){
								 $select_edit=mysql_query("SELECT * FROM efficient_letter_left WHERE id='".$_REQUEST['edit_id']."'");
	  $data_edit=mysql_fetch_assoc($select_edit);
                            ...
                            <form action="eficientleft-content.php" class="form-horizontal" method="post" enctype="multipart/form-data">
                             ...
                             <div class="control-group">
                                    ...
                                    <div class="controls">
                                        ...
                                        <input type="text" placeholder="Text" class="input-xlarge" name="text1" id="text1" value="<?php echo $data_edit['page_title']  ?>" required />
                            ...
                            <?php } else {
								$list_pages=mysql_query("SELECT * FROM efficient_letter_left");
                                 $count=mysql_num_rows($list_pages);
								?>
                            <!-- END FORM--><div class="btn-group">
                                         <button  onClick="window.location='eficientleft-content.php?action=add';" class="btn green" id="editable-sample_new">
                                             Add New <i class="icon-plus"></i>
                                         </button>
                                     </div>
                                     <p></p>
                                     <div class="widget red">
                                       <input type="hidden" name="tablename" id="tablename" value="efficient_letter_left"/>
                            <div class="widget-title">
...
while($data=mysql_fetch_object($list_pages))
{
 ?>
                                ...
                                <?php if($data->status==1)
		 {
			...
			<a class="btn btn-small btn-success" href="eficientleft-content.php?action=update_green&upd_id=<?php echo $data->id?>">Active</a></td><?php
		 }
		else
		 {
			?><td class="hidden-phone" >
			<a class="btn btn-small btn-danger" href="eficientleft-content.php?action=update_red&update_id=<?php echo $data->id?>">Deactive</a></td><?php
		 }?>
    
                                <td class="hidden-phone"><a class="btn btn-small btn-primary" href="eficientleft-content.php?action=edit&edit_id=<?php echo $data->id;?>"><i class="icon-pencil icon-white"></i> Edit</a></td>
...
$('.dell').live('click',function(){
  ...
  apprise("Are you sure you want to delete?", {'verify': true, 'animate': true, 'textYes': 'Yes', 'textNo': 'No' },
    function (r) {
  if (r) {
	  window.location.href='eficientleft-content.php?action=delete&dlt_id='+DelId;
	  }
   else
   {
    return false;
