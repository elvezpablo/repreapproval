...
{
	$update=mysql_query("UPDATE  footer_cont SET status='0' WHERE id='".$_REQUEST['upd_id']."' ");
		if($update)
	{
		header("LOCATION:footer-cont.php?msg=updated");
	}	
}
 
 if(isset($_REQUEST['action']) && $_REQUEST['action']=='update_red')
{
	$update=mysql_query("UPDATE  footer_cont SET status='1' WHERE id='".$_REQUEST['update_id']."' ");
	if($update)
	{
		header("LOCATION:footer-cont.php?msg=updated");
	}	
...
if(isset($_REQUEST['action']) && $_REQUEST['action']=='delete')
{
	...
	$delete=mysql_query("delete from  footer_cont  where id='".$_REQUEST['dlt_id']."'");
	if($delete)
	{
		header("LOCATION:footer-cont.php?msg=deleted");
	}	
}
...
if(isset($_REQUEST['submit']) && $_REQUEST['submit']=='Add')
{
	...
	$insert_pages=mysql_query("INSERT INTO  footer_cont set  image='".$img."', content='".addslashes($_REQUEST['content'])."',img_title='".addslashes($_REQUEST['img_title'])."',img_alt='".addslashes($_REQUEST['img_alt'])."'");
		if($insert_pages)
		{
			header("LOCATION:footer-cont.php?msg=Added");
		}
...
if(isset($_REQUEST['submit']) && $_REQUEST['submit']=="Update")
{
	...
	$insert_pages=mysql_query("update  footer_cont set  image='".$img."', content='".addslashes($_REQUEST['content'])."',img_title='".addslashes($_REQUEST['img_title'])."',img_alt='".addslashes($_REQUEST['img_alt'])."' where id='".$_REQUEST['edit_id']."'");
		if($insert_pages)
		{
			header("LOCATION:footer-cont.php?msg=updated");
		}
                ...
                <div class="span12">
                    <!-- BEGIN SAMPLE FORMPORTLET-->
                        ...
                        <div class="widget-body">
                            <form action="footer-cont.php" class="form-horizontal" method="post" enctype="multipart/form-data">
...
$sBasePath = substr( $sBasePath, 0, strpos( $sBasePath, "footer-cont.php"));
...
?>
                                ...
                                <div class="form-actions">
                                    ...
                                    <button type="button" class="btn" onClick="window.location='footer-cont.php';"><i class=" icon-remove"></i>Cancel</button>
                                </div>
                            </form>
                            </div></div>
                            <?php } else if(isset($_REQUEST['action']) and $_REQUEST['action']=='edit'){
								 $select_edit=mysql_query("SELECT * FROM  footer_cont WHERE id='".$_REQUEST['edit_id']."'");
...
?>
                            ...
                            <?php } else {
								$list_pages=mysql_query("SELECT * FROM  footer_cont");
                                 $count=mysql_num_rows($list_pages);
								?>
                            <!-- END FORM--><div class="btn-group">
                                        <!-- <button  onClick="window.location='footer-cont.php?action=add';" class="btn green" id="editable-sample_new">
                                             Add New <i class="icon-plus"></i>
                                         </button>-->
                                     </div>
                                     <p></p>
                                     <div class="widget red">
                                       <input type="hidden" name="tablename" id="tablename" value=" footer_cont"/>
                            <div class="widget-title">
...
while($data=mysql_fetch_object($list_pages))
{
 ?>
                                ...
                                <?php if($data->status==1)
		 {
			...
			<a class="btn btn-small btn-success" href="footer-cont.php?action=update_green&upd_id=<?php echo $data->id?>">Active</a></td><?php
		 }
		else
		 {
			?><td class="hidden-phone" >
			<a class="btn btn-small btn-danger" href="footer-cont.php?action=update_red&update_id=<?php echo $data->id?>">Deactive</a></td><?php
		 }?>
    
                                <td class="hidden-phone"><a class="btn btn-small btn-primary" href="footer-cont.php?action=edit&edit_id=<?php echo $data->id;?>"><i class="icon-pencil icon-white"></i> Edit</a></td>
...
$('.dell').live('click',function(){
  ...
  apprise("Are you sure you want to delete?", {'verify': true, 'animate': true, 'textYes': 'Yes', 'textNo': 'No' },
    function (r) {
  if (r) {
	  window.location.href='footer-cont.php?action=delete&dlt_id='+DelId;
	  }
   else
   {
    return false;
